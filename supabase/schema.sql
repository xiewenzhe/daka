...
begin
  ...
  if new.status = 'checked' and (new.checked_at < starts_at or new.checked_at > ends_at) then
    raise exception 'Check-in is only allowed within one hour before or after the scheduled time';
  end if;
